...
// ─────────────────────────────────────────────────────────────────────────────
// ПОСТОБРАБОТКА (лекция 08). Ровно 64 байта.
//
// Один и тот же буфер используют все четыре прохода, по элементу на проход:
// меняются только TexelSize и BlurDirection.
// ─────────────────────────────────────────────────────────────────────────────
struct alignas(16) PostConstants {
	DirectX::XMFLOAT2 TexelSize     = { 0.0f, 0.0f };   // 1/ширина, 1/высота источника
	DirectX::XMFLOAT2 BlurDirection = { 1.0f, 0.0f };   // (1,0) или (0,1)

	float Exposure         = 1.0f;
	float BloomThreshold   = 1.0f;     // яркость, выше которой начинается свечение
	float BloomIntensity    = 0.55f;
	float VignetteStrength = 0.35f;

	float    ChromaticAberration = 0.0015f;
	uint32_t ToneMapMode   = 1;        // 0 = выкл, 1 = Рейнхард, 2 = экспозиционный
	uint32_t DitherEnabled = 1;
	uint32_t Passthrough   = 0;        // 1 = копировать без обработки (режимы отладки)

	DirectX::XMFLOAT2 ScreenSize = { 1.0f, 1.0f };
